    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
      ...
      <div class="relative">
        ...
        <div ref="scrollContainer" 
          ...
          <div v-for="(tour, index) in discountedTours" 
               :key="tour.id"
      ...
      <div class="relative w-full md:w-[500px] h-[100vh] bg-black overflow-hidden"
        ...
        <div class="absolute top-0 left-0 right-0 z-20">
          ...
          <div class="px-2 pt-2 pb-4 flex gap-1">
            <div v-for="n in discountedTours.length" :key="n" 
          ...
          <div class="px-4 flex items-center justify-between">
            <div class="flex items-center gap-2">
              <div class="w-7 h-7 md:w-8 md:h-8 rounded-full overflow-hidden border border-white/20">
                <img :src="discountedTours[currentStoryIndex].image" 
                     :alt="discountedTours[currentStoryIndex].title"
                     class="w-full h-full object-cover">
              </div>
              <div class="flex items-center gap-2">
                <span class="text-white text-xs md:text-sm font-medium">{{ discountedTours[currentStoryIndex].title }}</span>
        ...
        <div class="w-full h-full">
          <TransitionGroup name="story" tag="div" class="w-full h-full">
            <img :key="currentStoryIndex" 
                 :src="discountedTours[currentStoryIndex].image" 
                 :alt="discountedTours[currentStoryIndex].title"
                 class="w-full h-full object-cover">
          </TransitionGroup>
        </div>
          ...
          <div class="flex flex-col gap-2 md:gap-3">
            ...
            <div class="flex items-center gap-3 md:gap-4 text-white/90">
              <div class="flex items-center gap-1 md:gap-1.5">
                ...
                <span class="text-sm md:text-base font-medium">{{ discountedTours[currentStoryIndex].rating }}</span>
              </div>
              ...
              <div class="flex items-center gap-1 md:gap-1.5">
                ...
                <span class="text-sm md:text-base font-medium">{{ discountedTours[currentStoryIndex].location }}</span>
              </div>
              <span class="text-white/30">•</span>
              <span class="text-base md:text-lg font-bold text-white">{{ discountedTours[currentStoryIndex].price }}</span>
            </div>
...
const openStory = (index) => {
  ...
  discountedTours.value[index].isViewed = true;
...
const nextStory = () => {
  if (currentStoryIndex.value < discountedTours.value.length - 1) {
    currentStoryIndex.value++;
    progressWidth.value = 0;
    startTime = performance.now();
    discountedTours.value[currentStoryIndex.value].isViewed = true;
    animationFrameId = requestAnimationFrame(updateProgressBar);
  } else {
    closeStory();
  }
...
const nextStoryImage = () => {
  if (currentStoryImageIndex.value < discountedTours.value[currentStoryIndex.value].storyImages.length - 1) {
...
const incrementViews = (index) => {
  discountedTours.value[index].views++;
  discountedTours.value[index].isViewed = true;
  saveViewedStory(discountedTours.value[index].id);
};
...
const toggleLike = () => {
  ...
  if (isLiked.value) {
    discountedTours.value[currentStoryIndex.value].likes++;
  } else {
    discountedTours.value[currentStoryIndex.value].likes--;
  }
};
...
const addComment = () => {
  if (newComment.value.trim()) {
    discountedTours.value[currentStoryIndex.value].comments.push({
      user: "Kullanıcı",
...
const loadViewedStories = () => {
  ...
  discountedTours.value.forEach(tour => {
